  ...
  <div class="user-page">
    ...
    <p class="query">
      ...
      <el-button
        ...
        @click="()=>this.$store.dispatch('updateCredentList')"
      ></el-button>
    </p>
    <el-table
      size="small"
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <div
            :key="scope.row.credentNo"
            @click="showPic(scope.row.credentPic, scope.row.credentNo)"
            class="authPic"
          >
            <img
              :src="'http:'+picServerPath+scope.row.credentPic"
              alt="查看"
            >
          </div>
        ...
        <template slot-scope="scope">
          ...
          >
            <el-button
              type="success"
              size="small"
              @click="credentAdmit(scope)"
              plain
            >通过</el-button>
            <el-button
              type="danger"
              size="small"
              @click="refuseClick(scope)"
              plain
            >拒绝</el-button>
          </p>
          <p
    ...
    <el-dialog
      :title="'证件号：'+identificationNum"
      :width="'940px'"
      :visible.sync="identifPicVisible"
    >
      ...
      auditCredent(scope.row.userId, "1")
        .then(data => {
          if (data.code == 200) {
            ...
            addCredentNote(scope.row.userId, "", "通过认证。").catch(() => {});
            this.$store.dispatch("updateCredentList");
          } else {
            this.$notify.error({
              title: "错误",
